...
def preprocessing(achor_x, achor_y, diff_y):
    ...
    return Variable(achor_x[idx]), pack_padded_sequence(a, len_a, batch_first=True), pack_padded_sequence(d, len_d, batch_first=True)
...
def preprocessing2(achor_x, achor_y, diff_x):
    ...
    a, idx = argsort(a)
    a, len_a = pad_packed_sequence(pack_sequence(a), batch_first=True)
    return pack_padded_sequence(a, len_a, batch_first=True), Variable(achor_x[idx]), Variable(diff_x[idx])
...
if __name__ == "__main__":
    ...
    print('-' * 89)
    print("Loading data...")
    ntokens, train_x, train_y, train_ldr, dev_ldr, test_ldr, word_to_onehot_map, i_to_word_map = data_loader()
    print('-' * 89)
    print("Data loaded")
    print('-' * 89)
